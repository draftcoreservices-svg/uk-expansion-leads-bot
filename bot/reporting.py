...
def build_html(meta: Dict[str, Any], leads: List[Dict[str, Any]], backfill_count: int) -> str:
    ...
    sponsor_new = meta.get("sponsor_new_count", 0)
    ...
    <div class="card">
      ...
      <div style="margin-top:10px" class="grid">
        <div class="kv"><div class="k">New sponsors detected (since baseline)</div><div class="v">{sponsor_new}</div></div>
...
def build_csv(leads: List[Dict[str, Any]]) -> str:
    ...
    out = io.StringIO()
    w = csv.writer(out)
    w.writerow(["bucket","company_name","company_number","case_type","visa_hint","score","incorporated","reg_town","website","emails","phones","why","source"])
    for l in leads:
        w.writerow([
            l.get("bucket", ""),
            l.get("company_name", ""),
