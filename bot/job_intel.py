...
def score_hiring_intent(
    ...
    for q in queries[:3]:
        if serp_budget.get("cap", 0) and serp_budget["calls"] >= serp_budget["cap"]:
            break
        serp_budget["calls"] += 1
        results = serp_search(session, q, serp_key, num=cfg.serp_jobs_results_per_query)
    ...
    if urls:
        likely_jobs = [u for u in urls if re.search(r"(careers|jobs|vacan|recruit|indeed|workday|greenhouse|lever|smartrecruiters)", u, flags=re.I)]
        if likely_jobs:
            score += 4
