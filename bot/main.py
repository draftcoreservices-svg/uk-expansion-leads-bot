...
def _officer_signals(officers: list[dict], cfg: Config) -> dict:
    ...
    for o in officers or []:
        ...
        if nat:
            nats.add(nat.title())
        # nationality isn't always a country name; use a loose heuristic:
        # if not UK/English/Scottish etc treat as non-UK
...
def _psc_signals(psc_items: list[dict], cfg: Config) -> dict:
    ...
    for p in psc_items or []:
        ...
        is_corp = any(
            k in kind
            for k in ["corporate-entity", "legal-person", "corporate", "other-registrable-person"]
        )
        if is_corp:
            if name:
...
def main():
    ...
    try:
        ...
        if sponsor_baselined != "1":
            for row, f in filtered:
                ...
                db.upsert_sponsor_row(
                    conn, k, f["name"], f["town"], f["county"], f["route"], f["subroute"], run_ts_iso
                )
    ...
    items = ch.advanced_incorporated(
        http, inc_from, inc_to, size=100, max_total=cfg.ch_max_results_total
    )
    ...
    for idx, it in enumerate(items, start=1):
        ...
        if not looks_like_subsidiary_name(company_name):
            # leave as pass for now (keep behaviour), but you can add stricter gating later if needed
            pass
    ...
    for _row, f, row_key in sponsor_new:
        ...
        if mapped:
            ...
            company_number, match_score = best_ch_match_for_sponsor(
                http, f["name"], f["town"], timeout=cfg.ch_search_timeout
            )
    ...
    for ld in leads:
        ...
        ld["bucket"] = sr.bucket
        ld["why"] = sr.why
        ld["visa_hint"] = visa_hint(
            "SPONSOR_REGISTER" if "SPONSOR_REGISTER" in (ld.get("source", "")) else "COMPANIES_HOUSE",
            ld.get("sponsor_route", ""),
            sr.score,
        )
    ...
    def should_enrich(ld: dict) -> bool:
        # enrich Hot + Medium, and sponsor-based leads even if watch
    ...
    db.run_finish(
        conn,
        run_id,
        sponsor_new=len(sponsor_new),
        ch_candidates=len(ch_candidates),
        serp_calls=serp_budget["calls"],
        verified_sites=verified_sites,
    )
    conn.commit()
